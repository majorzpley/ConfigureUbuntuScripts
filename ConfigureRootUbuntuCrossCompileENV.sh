...
tftp_configure(){
    ...
    tftp_file=/root/tftpboot  # 定义 TFTP 服务器的根目录路径
    ...
    grep "/root/tftpboot" /etc/default/tftpd-hpa 1>/dev/null  # 检查配置文件中是否已经包含 TFTP 目录配置
    if [ $? -ne 0 ];then  # 如果配置文件中没有包含 TFTP 目录配置
        # sed  -i '$a\TFTP_DIRECTORY="/home/'"$user_name"'/tftpboot"' /etc/default/tftpd-hpa  # 添加 TFTP 目录配置
        sed  -i '$a\TFTP_DIRECTORY="/root/tftpboot"' /etc/default/tftpd-hpa  # 添加 TFTP 目录配置
...
nfs_configure() {
    ...
    nfs_file=/root/nfs_rootfs  # 定义 NFS 服务器的根目录路径
    ...
    grep "/root/" /etc/exports 1>/dev/null  # 检查配置文件中是否已经包含 NFS 目录配置
    if [ $? -ne 0 ];then  # 如果配置文件中没有包含 NFS 目录配置
        ...
        sed -i '$a\/root/  *(rw,nohide,insecure,no_subtree_check,async,no_root_squash)' /etc/exports   
    fi
